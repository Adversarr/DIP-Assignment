...
class LitModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_g = self.criterion(self.adversarial(outputs), real_label)
        loss_g_mse = F.l1_loss(outputs, targets)
        self.log('loss_g', loss_g)
        self.log('loss_g_l1', loss_g_mse)
        self.manual_backward(loss_g + loss_g_mse * 5)
        o_model.step()
        o_model.zero_grad()
        ...
        fake = self.adversarial(outputs.detach())
        real = self.adversarial(targets)
        loss_d = (self.criterion(fake, fake_label) + self.criterion(real, real_label)) * 0.5
        ...
        if self.current_epoch % 10 == 0 and batch_idx == 0:
            ...
            grid = tensor_to_image(torchvision.utils.make_grid(torch.cat([inputs[:b], targets[:b], outputs[:b]], dim=0), nrow=b))
            run = self.logger.run_id
            os.makedirs('train_results', exist_ok=True)
            file_name = f'train_results/epoch_{self.current_epoch}.png'
    ...
    def configure_optimizers(self):
        ...
        b1 = 0.5
        b2 = 0.999
        o1 = optim.AdamW(self.model.parameters(), lr=lr, betas=(b1, b2))
        o2 = optim.AdamW(self.adversarial.parameters(), lr=lr, betas=(b1, b2))
        return [{
            'optimizer': o1,
        }, {
            'optimizer': o2,
...
class DataModule(L.LightningDataModule):
    ...
    def val_dataloader(self):
        return torch.utils.data.DataLoader(self.val, batch_size=self.batch_size * 8, shuffle=False)
